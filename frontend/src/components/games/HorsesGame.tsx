...
export default function HorsesGame({ code: _code }: Props) {
  ...
  useEffect(() => {
    const onState = (s: unknown) => {
      const hs = s as HorsesState;
      setState(hs);
      if (hs.phase === 'betting') { setPositions(new Array(8).fill(0)); setWinnerId(null); setResult(null); setMyBets({}); }
    };
    const onTimer = (d: unknown) => {
      const { timeLeft } = d as { timeLeft: number };
      setState(prev => prev ? { ...prev, bettingTimeLeft: timeLeft } : prev);
    };
    const onFrame = (d: unknown) => {
      const { positions: pos, winnerId: wId } = d as { positions: number[]; winnerId: number | null };
      setPositions([...pos]);
      if (wId !== null) setWinnerId(wId);
    };
    const onResult = (data: unknown) => {
      const r = data as { winnerId: number; winnerName: string; winnings: Record<string, number> };
      setResult(r);
      if (myId && r.winnings[myId]) {
        toast.success(`🏆 ¡Ganaste $${r.winnings[myId].toLocaleString()}!`, { duration: 4000 });
      } else {
        toast.error('Mala suerte en esta carrera', { duration: 2000 });
      }
    };
    on('horses:state', onState);
    on('horses:timer', onTimer);
    on('horses:frame', onFrame);
    on('horses:result', onResult);
  ...
  return (
    <div className="max-w-4xl mx-auto">
        ...
        <div>
          ...
          {state.phase === 'result' && result && (
            <span className="text-yellow-400 font-bold">
              🏆 Ganador: <span style={{ color: state.horses.find(h => h.id === result.winnerId)?.color }}>{result.winnerName}</span>
            </span>
          )}
        </div>
        {state.phase === 'result' && <span className="text-gray-400 text-sm">Nueva carrera en unos segundos...</span>}
      </div>
      ...
      <div className="felt-table p-4 mb-4 overflow-hidden rounded-xl" style={{ background: 'linear-gradient(180deg, #1a3d1a 0%, #0d2b0d 100%)' }}>
          ...
          return (
            <div key={horse.id} className="mb-2">
              <div className="flex items-center gap-2 mb-1">
                ...
                <span className="text-xs font-bold" style={{ color: horse.color, minWidth: 100 }}>{horse.name}</span>
                <span className="text-xs text-gray-500">x{horse.odds.toFixed(1)}</span>
                {myBetOnHorse > 0 && <span className="text-xs text-yellow-400">💰${myBetOnHorse.toLocaleString()}</span>}
              ...
              <div className="relative h-7 rounded-full border border-green-900" style={{ background: 'rgba(0,0,0,0.3)' }}>
                ...
                <motion.div className="absolute top-1/2 -translate-y-1/2 text-xl"
                  style={{ left: `${Math.max(0, pct - 4)}%` }}
                  transition={{ ease: 'linear' }}>🏇</motion.div>
        ...
        <div className="flex justify-end mt-1"><span className="text-gray-600 text-xs">🏁 Meta</span></div>
      </div>
      ...
      {state.phase === 'betting' && (
        <div className="glass-card p-4">
          <div className="flex items-center gap-3 mb-4">
            ...
            {CHIP_VALUES.map((v, i) => (
              <button key={v} onClick={() => setSelectedChip(v)} className="chip w-12 h-12"
                style={{
                  background: CHIP_COLORS[i], color: v === 100 ? '#1a1a2e' : 'white',
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
            {state.horses.map(horse => (
              <button key={horse.id} onClick={() => bet(horse.id)}
                className={`p-3 rounded-xl text-left transition-all border ${myBets[horse.id] ? 'border-yellow-400 bg-yellow-400/10' : 'border-white/10 hover:border-white/30'}`}>
                <div className="flex items-center gap-2 mb-1">
                ...
                <div className="flex justify-between items-center">
                  ...
                  {myBets[horse.id] && <span className="text-green-400 text-xs">${myBets[horse.id].toLocaleString()}</span>}
                </div>
              </button>
            ))}
          </div>
