...
export default function PokerGame({ code: _code }: Props) {
  const { emit, on, off, myId, balance } = useRoom();
  const [state, setState] = useState<PokerState | null>(null);
  const [raiseAmount, setRaiseAmount] = useState(0);
  const [myHoleCards, setMyHoleCards] = useState<PokerState['players'][0]['holeCards']>([]);

  useEffect(() => {
    const onState = (s: unknown) => {
      const ps = s as PokerState;
      setState(ps);
      if (ps.phase === 'waiting') setMyHoleCards([]);
    };
    const onHole = (d: unknown) => {
      const { holeCards } = d as { holeCards: PokerState['players'][0]['holeCards'] };
      setMyHoleCards(holeCards);
    };
    on('poker:state', onState);
    on(`poker:hole:${myId}`, onHole);
    return () => { off('poker:state', onState); off(`poker:hole:${myId}`, onHole); };
  }, [on, off, myId]);

  const myPlayer = state?.players.find(p => p.socketId === myId);
  const isMyTurn = state?.currentPlayerSocketId === myId && !myPlayer?.folded && !myPlayer?.allIn;
  ...
  const action = (type: string, amount?: number) => emit('poker:action', { playerId: myId, action: type, amount });
  const startHand = () => emit('poker:start');
  const nextHand = () => emit('poker:next-hand');
  ...
  return (
    <div className="max-w-4xl mx-auto">
        ...
        {(state?.players ?? []).map((player, i) => {
          ...
          return (
            <motion.div key={player.socketId}
              ...
              <div className={`glass-card px-2 py-1 min-w-20 ${isMe ? 'border-yellow-400/60' : ''} ${isCurrent ? 'ring-2 ring-green-400' : ''}`}>
                <p className="text-xs font-bold text-white truncate max-w-20">{isMe ? '👤 Tú' : player.alias}</p>
                <p className="text-yellow-400 text-xs">${player.balance.toLocaleString()}</p>
                {player.bet > 0 && <p className="text-green-400 text-xs">${player.bet.toLocaleString()}</p>}
                {player.allIn && <p className="text-red-400 text-xs font-bold">ALL IN!</p>}
                {player.folded && <p className="text-gray-500 text-xs">FOLD</p>}
                {player.lastAction && !player.folded && <p className="text-blue-400 text-xs capitalize">{player.lastAction}</p>}
                <div className="flex gap-1 justify-center mt-1">
                  {(displayCards ?? []).map((card, ci) => <CardComponent key={ci} card={card} size="sm" />)}
                  {player.handSize > 0 && (displayCards?.length ?? 0) === 0 && (
      ...
      {/* Winners */}
      <AnimatePresence>
        {state?.phase === 'showdown' && (state?.winners?.length ?? 0) > 0 && (
      ...
      {myHoleCards.length > 0 && (
        <div className="glass-card p-4 mt-3 flex items-center gap-4">
          <div>
            ...
            <div className="flex gap-2">
              {myHoleCards.map((c, i) => <CardComponent key={i} card={c} size="lg" />)}
            </div>
          </div>
          {myPlayer?.bet ? (
      ...
      <div className="glass-card p-4 mt-3">
        {state?.phase === 'waiting' && (
          <div className="text-center">
            ...
            <button onClick={startHand} disabled={(state?.players?.length ?? 0) < 2}
              className="casino-btn px-10 py-3 rounded-lg disabled:opacity-50">
              🃏 Iniciar Mano
            </button>
          </div>
        )}

        {!state && <p className="text-center text-gray-500">Esperando al host...</p>}
        ...
        {isMyTurn && state?.phase !== 'waiting' && state?.phase !== 'showdown' && (
          <div>
            ...
            <div className="flex gap-2 flex-wrap justify-center mb-3">
              <button onClick={() => action('fold')} className="px-6 py-2 rounded-lg font-bold bg-red-800 hover:bg-red-700 text-white transition-colors">🃏 Fold</button>
              {canCheck ? (
                <button onClick={() => action('check')} className="px-6 py-2 rounded-lg font-bold bg-blue-800 hover:bg-blue-700 text-white transition-colors">✓ Check</button>
              ) : (
                <button onClick={() => action('call')} disabled={callAmount <= 0}
                  className="px-6 py-2 rounded-lg font-bold bg-blue-800 hover:bg-blue-700 text-white transition-colors">📞 Call ${callAmount.toLocaleString()}</button>
              )}
              <button onClick={() => action('allin')} className="px-6 py-2 rounded-lg font-bold bg-purple-800 hover:bg-purple-700 text-white transition-colors">🔥 All In</button>
            </div>
            <div className="flex gap-2 items-center justify-center">
              ...
              <button onClick={() => action('raise', raiseAmount)} disabled={raiseAmount < (state?.minRaise ?? 0)}
                className="casino-btn px-6 py-2 rounded-lg text-sm">↑ Raise</button>
            </div>
        ...
        {!isMyTurn && state?.phase !== 'waiting' && state?.phase !== 'showdown' && (
          <p className="text-center text-gray-400">
            {myPlayer?.folded ? '🃏 Te retiraste de esta mano' : `Turno de: ${state?.players.find(p => p.socketId === state?.currentPlayerSocketId)?.alias ?? '...'}`}
          </p>
        )}

        {state?.phase === 'showdown' && (
          <div className="text-center">
            <button onClick={nextHand} className="casino-btn px-10 py-3 rounded-lg">▶ Siguiente Mano</button>
          </div>
        )}
      </div>
    </div>
