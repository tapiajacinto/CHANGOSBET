import type { FootballState, FootballMatch, FootballTeam, FootballBet } from '@/types';
...
function calcOdds(home: FootballTeam, away: FootballTeam) {
  ...
  const m = 1.05;
  return {
    home: Math.round(homeBase * total * m * 100) / 100,
    draw: Math.round(drawBase * total * m * 100) / 100,
    away: Math.round(awayBase * total * m * 100) / 100,
  };
}

function generateMatches(count: number): FootballMatch[] {
  const teams = [...TEAMS].sort(() => Math.random() - 0.5).slice(0, count * 2);
  return Array.from({ length: count }, (_, i) => {
    ...
    return {
      ...
      overOdds: 1.85, underOdds: 1.95, bttsYesOdds: 1.72, bttsNoOdds: 2.0,
...
export function createFootballState(): FootballState {
  return { phase: 'betting', matches: generateMatches(6), bets: [], bettingTimeLeft: 25 };
}

export function simulateMatch(match: FootballMatch) {
  const events: { minute: number; type: string; team: 'home' | 'away'; description: string }[] = [];
  const total = match.home.strength + match.away.strength;
  const goals = Math.round(Math.random() * 2.5 + 0.5);
  const minutes = Array.from({ length: goals }, () => Math.floor(Math.random() * 90) + 1).sort((a, b) => a - b);

  for (const minute of minutes) {
    const isHome = Math.random() < match.home.strength / total;
    const team: 'home' | 'away' = isHome ? 'home' : 'away';
    const scorer = isHome ? match.home.shortName : match.away.shortName;
    events.push({ minute, type: 'goal', team, description: `⚽ GOL! ${scorer} marca al ${minute}'` });
    if (isHome) match.homeScore++; else match.awayScore++;
  }
  if (Math.random() < 0.3) {
    events.push({ minute: Math.floor(Math.random() * 90) + 1, type: 'yellow_card', team: Math.random() < 0.5 ? 'home' : 'away', description: '🟨 Tarjeta amarilla' });
  }
  match.status = 'finished'; match.minute = 90;
  return events.sort((a, b) => a.minute - b.minute);
}

export function resolveFootballBets(bets: FootballBet[], matches: FootballMatch[]): Map<string, number> {
