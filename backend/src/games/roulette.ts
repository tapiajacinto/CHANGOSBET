import { RouletteState, RouletteBet, RouletteBetType } from '../types';
...
export function createRouletteState(): RouletteState {
  return {
    phase: 'betting',
    bets: [],
    winningNumber: null,
...
function calculatePayout(bet: RouletteBet, winning: number): number {
  ...
  const hit = numbers.includes(winning);
  if (!hit) return 0;
...
export function buildBetNumbers(type: RouletteBetType, data?: number[]): number[] {
  switch (type) {
    ...
    case 'split': return data ?? [];
    case 'street': return data ?? [];
    case 'corner': return data ?? [];
    case 'sixline': return data ?? [];
    default: return [];
  }
}
