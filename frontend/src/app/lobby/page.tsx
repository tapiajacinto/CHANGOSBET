...
export default function LobbyPage() {
  ...
  useEffect(() => {
    ...
    const onError = ({ message }: any) => toast.error(message);
    socket.on('room:joined', onJoined);
    socket.on('error', onError);
    return () => { socket.off('room:joined', onJoined); socket.off('error', onError); };
  }, [socket, router]);

  const createRoom = () => {
    if (!socket || !user) return;
    if (!roomName.trim()) { toast.error('Ponele un nombre a la sala'); return; }
    socket.emit('room:create', { name: roomName.trim(), game: selectedGame, alias: user.alias });
  };
            ...
            <div className="space-y-4">
              <div>
                ...
                <input value={roomName} onChange={e => setRoomName(e.target.value)}
                  ...
                  className="w-full px-4 py-3 rounded-lg border border-yellow-400/30 text-white
              ...
              <button onClick={createRoom} disabled={!connected}
                className="casino-btn w-full py-4 text-base rounded-lg mt-2">
                🎲 Crear Sala y Copiar Código
              </button>
            </div>
          </motion.div>
        ) : (
            ...
            <div className="space-y-4">
              <div>
                ...
                <input value={joinCode} onChange={e => setJoinCode(e.target.value.toUpperCase())}
                  ...
                  className="w-full px-4 py-3 rounded-lg border border-yellow-400/30 text-white text-center
                             text-2xl tracking-widest focus:outline-none focus:border-yellow-400 transition-colors"
                  style={{ background: 'rgba(10,10,20,0.8)' }} />
              </div>
              <button onClick={joinRoom} disabled={!connected}
                className="casino-btn w-full py-4 text-base rounded-lg">
                🚪 Entrar a la Sala
              </button>
            </div>
            <div className="mt-6 p-4 rounded-lg bg-blue-900/20 border border-blue-700/30">
              <p className="text-blue-300 text-sm text-center">
                Pedile el código a quien creó la sala. Solo quienes lo tengan pueden entrar.
              </p>
            </div>
          </motion.div>
        )}
